...
(define (get-register-contents machine register) (get-contents (get-register machine register-name)))
...
(define (assign-value-exp assign-instruction) (cdr assign-instruction))
  ...
  (let ((condition (test-condition inst)))
    (if (operation-exp? condition)
      ...
      (lambda ()
          ...
          (advance-pc pc)))
        (error "Bad Test instruction: ASSEMBLE" int))))
...
(let ((action (perform-action inst)))
  (if (operation-exp? action) (let ((action-proc
                (make-operation-exp
...
(define (register-exp? exp) (tagged-list? exp 'reg)) (define (register-exp-reg exp) (cadr exp))
(define (constant-exp? exp) (tagged-list? exp 'const)) (define (constant-exp-value exp) (cadr exp))
(define (label-exp? exp) (tagged-list? exp 'label)) (define (label-exp-label exp) (cadr exp))
